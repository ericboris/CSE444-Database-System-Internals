...
//    int pageSize;
    ...
    */
    public void rollback(TransactionId tid)
        ...
        tidSet.add(tid.getId());
        undo(tidSet, tidToFirstLogRecord.get(tid.getId()));
    }
    ...
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                recoveryUndecided = false;
                HashSet<Long> tidSet = new HashSet<>();
                long prevCheckpoint = raf.readLong();

                if (prevCheckpoint != NO_CHECKPOINT_ID) {
                    raf.seek(prevCheckpoint);
                ...
                // Then handle undo.
                long minOffset = logFile.length();
                for (long tid: tidSet){
                    minOffset = Math.min(minOffset, tidToFirstLogRecord.get(tid));
                    tidToFirstLogRecord.remove(tid);
                }

                undo(tidSet, minOffset);
                force();
            }
        }
    }

    public void undo(HashSet<Long> tidSet, long offset)
            throws NoSuchElementException, IOException {
        synchronized(this) {
            preAppend();
            ...
            while (true) {
                try {
                    ...
                    switch (type) {
                        ...
                        case CHECKPOINT_RECORD:
                            raf.skipBytes(2 * raf.readInt() * LONG_SIZE);
                            break;
            ...
            while (!pageStack.isEmpty()) {
                Page curr = pageStack.pop();
                HeapFile hf = (HeapFile)Database.getCatalog().getDatabaseFile(curr.getId().getTableId());
                Database.getBufferPool().discardPage(curr.getId());
                hf.writePage(curr);
            }
        }
    }

    private void redo(HashSet<Long> tidSet) throws IOException {
        ...
        switch (type) {
            ...
            case UPDATE_RECORD:
                Page prev = readPageData(raf);
                Page next = readPageData(raf);
                Database.getBufferPool().discardPage(prev.getId());
                HeapFile hf = (HeapFile)Database.getCatalog().getDatabaseFile(prev.getId().getTableId());
                hf.writePage(next);
                break;
